        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-terracoin-address
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CTerracoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CTerracoinAddress(address).ToString());
}
