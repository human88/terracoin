...
{
#ifdef QT_GUI
    // ensure we leave the Qt main loop for a clean GUI exit (Shutdown() is called in terracoin.cpp afterwards)
...
void Shutdown(void* parg)
{
    ...
    RenameThread("terracoin-shutoff");
    ...
    if (fFirstThread)
    {
        ...
        Sleep(50);
        printf("Terracoin exited\n\n");
        fExit = true;
#ifndef QT_GUI
        // ensure non-UI client gets exited here, but let Terracoin-Qt reach 'return 0;' in terracoin.cpp
        exit(0);
#endif
    }
    else
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/terracoin.conf are parsed in qt/terracoin.cpp's main()
        ...
        {
            // First part of help message is specific to terracoind / RPC client
            std::string strUsage = _("Terracoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  terracoind [options]                     " + "\n" +
                  "  terracoind [options] <command> [params]  " + _("Send command to -server or terracoind") + "\n" +
                  "  terracoind [options] help                " + _("List commands") + "\n" +
                  "  terracoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "terracoin:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect terracoind signal handlers
    noui_connect();
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: terracoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: terracoind.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 13333 or testnet: 23333)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 13332 or testnet: 23332)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Terracoin Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Terracoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Terracoin is probably already running."), strDataDir.c_str()));
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Wallet needed to be rewritten: restart Terracoin to complete") << "\n";
