...
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            ...
            // Note: CTerracoinSecret::SetString tests isValid, whereas CTerracoinAddress does not!
...
BOOST_AUTO_TEST_CASE(base58_keys_valid_gen)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        else
        {
            ...
            CTerracoinAddress addrOut;
            BOOST_CHECK_MESSAGE(boost::apply_visitor(CTerracoinAddressVisitor(&addrOut), dest), "encode dest: " + strTest);
            BOOST_CHECK_MESSAGE(addrOut.ToString() == exp_base58string, "mismatch: " + strTest);
        }
    }

    // Visiting a CNoDestination must fail
    CTerracoinAddress dummyAddr;
    CTxDestination nodest = CNoDestination();
    BOOST_CHECK(!boost::apply_visitor(CTerracoinAddressVisitor(&dummyAddr), nodest));
